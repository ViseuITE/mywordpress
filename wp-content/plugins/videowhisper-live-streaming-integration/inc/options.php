...
<form method="post" action="<?php echo wp_nonce_url( esc_url( $_SERVER['REQUEST_URI'] ) , 'vwsec' ); ?>">
		...
		switch ( $active_tab ) {
			...
			case 'logs':
				...
				echo '<p>Find all logs at ' . esc_html($options['uploadsPath']) . '/logs </p>';
				break;
...
<br> - IP Cameras feature needs to be enabled from <a href="admin.php?page=live-streaming&tab=features">Channel Features</a> for broadcaster user roles that need to use this
				...
				if ( $tryStream = intval( $_GET['tryStream'] ?? 0 ) )
				{
					...
					if ( ! $roomPost ) {
						...
						echo '<a class="button" href="' . esc_url('post.php?action=edit&post=' . $tryStream) . '">Edit Channel</a>';
...
{			
			...
			if ( ! file_exists( $filename ) ) {
				...
				echo 'Snapshot could not be retrieved from ' . esc_html($addressProtocol) . ': ' . esc_html($address) . esc_html($devInfo);
			...
			echo '<br>A snapshot was retrieved: <br> <IMG class="ui rounded image big" SRC="' . esc_url($previewSrc) . '"><br>' . esc_html($devInfo);
			...
			flush();
				...
				echo '<br>Command: ' . esc_html($command);
				echo '<br><pre>' . esc_html($output) . '</pre>';
				...
				if ( count( $files ) ) {
					foreach ( $files as $file ) {
						...
						if ( ! $found ) {
							...
							echo '<a href="' . get_permalink( $found ). '">#' . esc_html( $found ). '</a>';
							};
					}
				} else {
