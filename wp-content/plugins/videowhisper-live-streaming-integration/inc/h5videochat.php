...
trait H5Videochat {
	...
	//self::requirementMet( 'rtmp_status' );
		...
		$streams= json_decode( stripslashes( sanitize_textarea_field( $_POST['streams'] ?? '') ), true ) ;
			...
			$resultStreams[$stream] = [ 'postID' => $postID ];
				...
				$r = $room;
                    ...
                    $session = self::sessionUpdate( $stream, $room, 1, 2, 0, 0, $userID, $postID, $options); // not strict in case user also in web chat
                        ...
                        //update channel
                        $sqlC = $wpdb->prepare("SELECT * FROM $table_channels WHERE name=%s LIMIT 1", $session->room);
...
//app functions
		...
		static function sessionValid( $sessionID, $userID, $broadcaster = false ) {
            ...
            $sqlS    = $wpdb->prepare("SELECT * FROM $table_sessions WHERE id=%d AND uid=%d AND status < 2 LIMIT 1", $sessionID, $userID);
...
static function appTipOptions( $options = null ) {
   ...
   return array();
    ...
    // all requests, including login:
			...
			case 'tick':
                ...
                case 'recorder_upload':
                    ...
                    if ( ! $response['warning'] && $scenario == 'chat' ) {
                        ...
                        $sql = $wpdb->prepare("INSERT INTO `$table_chatlog` ( `username`, `room`, `room_id`, `message`, `mdate`, `type`, `user_id`, `meta`, `private_uid`) VALUES (%s, %s, %d, %s, %d, %d, %d, %s, %d)", $messageUser, $roomName, $roomID, $messageText, $ztime, 2, $userID, $metaS, $privateUID);
                        $wpdb->query( $sql );
                ...
                case 'user_ban':
                    ...
                    case 'file_upload':
                        ...
                        case 'tip':
                            ...
                            $tip = isset( $_POST['tip'] ) ? (array) $_POST['tip'] : array(); // array elements sanitized individually as used
        ...
        $cndNotification = $wpdb->prepare("AND (type < 3 OR (type=3 AND user_id=%d AND username=%s))", $userID, $userName); // chat message or own notification (type 3)
        ...
        $sql = $wpdb->prepare("SELECT * FROM `$table_chatlog` WHERE room=%s $cndNotification $cndPrivate $cndTime ORDER BY mdate DESC LIMIT 0,100", $roomName); // limit to last 100 messages, until processed date
...
static function appRoomUsers( $post, $options ) {
    ...
    $sqlS = [
        $wpdb->prepare("SELECT * FROM `$table_sessions` WHERE rid=%d AND status = 1 ORDER BY sdate ASC", $post->ID),
        $wpdb->prepare("SELECT * FROM `$table_viewers` WHERE rid=%d AND status = 1 ORDER BY sdate ASC", $post->ID)
    ];
...
static function appRoomMedia( $post, $session, $options ) {
	...
	// room donations goal
        ...
        static function notificationMessage( $message, $session, $privateUID = 0, $meta = null ) {
            ...
            $sql = $wpdb->prepare("INSERT INTO `$table_chatlog` ( `username`, `room`, `room_id`, `message`, `mdate`, `type`, `user_id`, `meta`, `private_uid`) VALUES (%s, %s, %d, %s, %d, %d, %d, %s, %d)", $session->username, $session->room, $session->rid, $message, $ztime, 3, $session->uid, $metaS, $privateUID);
            $wpdb->query($sql);
        ...
        static function autoMessage( $message, $session, $privateUID = 0, $meta = null ) {
            ...
            $sql = $wpdb->prepare("INSERT INTO `$table_chatlog` ( `username`, `room`, `room_id`, `message`, `mdate`, `type`, `user_id`, `meta`, `private_uid`) VALUES (%s, %s, %d, %s, %d, %d, %d, %s, %d)", $session->username, $session->room, $session->rid, $message, $ztime, 2, $session->uid, $metaS, $privateUID);
            $wpdb->query( $sql );
