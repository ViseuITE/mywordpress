...
HTMLCODE;
	...
	static function respond( $msg, $request = '' ) {		
			echo $msg; 	// outputs ajax setup forms, parameters escaped prior to call
			exit;
		}
...
HTMLCODE;
			...
			if ( ! file_exists( $filename ) ) {
				...
				$respondCode .= '<br>Snapshot could not be retrieved from ' . esc_html( $addressProtocol ) . ': ' . esc_html( $address ) . esc_html( $devInfo ) . $retryCode;
...
HTMLCODE;
			...
			global $wpdb;
			$existID = $wpdb->get_var( $wpdb->prepare( "SELECT ID FROM $wpdb->posts WHERE `post_title` = %s AND `post_type` = %s LIMIT 0,1", $label, $options['custom_post'] ) ); // same name, diff than postID

			if ( $existID ) {
				if ( $postID != $existID ) {
					self::respond( 'A different channel with this name already exists:' . esc_html( $label) );
				}
			}
			...
			if ( $postID ) {
				...
				update_post_meta( $postID, 'creatorFc', 'vwls_stream_setup' );
			}

			// add to channels table if missing
			$table_channels = $wpdb->prefix . 'vw_lsrooms';

			$sql     = $wpdb->prepare( "SELECT * FROM $table_channels WHERE name = %s", $label );
			$channel = $wpdb->get_row( $sql );

			if ( $channel ) {
				if ( $channel->owner != $userID ) {
					self::respond( 'Channel name already used by different owner:' . esc_html( $label ) );
				}
			}
