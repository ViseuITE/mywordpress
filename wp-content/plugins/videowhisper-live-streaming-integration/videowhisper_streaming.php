	...
	class VWliveStreaming {
		...
		*/
		...
		static function before_delete_post( $postID ) {
			...
			global $wpdb;
			$table_channels = $wpdb->prefix . 'vw_lsrooms';
			$sql = $wpdb->prepare( "DELETE FROM $table_channels WHERE name = %s", $room );
	...
	<style type="text/css">
		...
		static function roomURL( $room ) {
			...
			if ( $options['channelUrl'] == 'post' ) {
				...
				$postID = $wpdb->get_var( $wpdb->prepare( "SELECT MAX(ID) FROM $wpdb->posts WHERE post_title = %s and post_type='channel' LIMIT 0,1", sanitize_file_name( $room ) ) );
		...
		static function count_user_posts_by_type( $userid, $post_type = 'channel' ) {
			...
			$count = $wpdb->get_var( "SELECT COUNT(*) FROM $wpdb->posts $where" ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared : $where obtained with get_posts_by_author_sql()
		...
		static function channelInvalid( $channel, $broadcast = false ) {
			...
			$sql      = $wpdb->prepare( "SELECT * FROM $table_channels WHERE name = %s", $channel );
			...
			if ( ! $channelR ) {
				if ( $broadcast ) {
					...
					//get post
					$postID = $wpdb->get_var( $wpdb->prepare( 'SELECT ID FROM ' . $wpdb->posts . ' WHERE post_title = %s and post_type=%s LIMIT 0,1', $channel, $options['custom_post'] ) );
					if ( $postID ) {
						$post = get_post( $postID );
						if ($post)
						{
							...
							$ztime = time();
							$sql = $wpdb->prepare( "INSERT INTO $table_channels ( `owner`, `name`, `sdate`, `edate`, `rdate`,`status`, `type`) VALUES ( %d, %s, %d, %d, %d, %d, %d )", $post->post_author, $channel, $ztime, $ztime, $ztime, 0, 1 );
							$wpdb->query( $sql );
						}
					}
		...
		else
		{
			...
			$postID = $wpdb->get_var( $wpdb->prepare( "SELECT MAX(ID) FROM $wpdb->posts WHERE post_title = %s and post_type='channel' LIMIT 0,1", $channel ) );
		...
		static function videowhisper_channel_user() {
			...
			global $wpdb;
			$postID = $wpdb->get_var( $wpdb->prepare( "SELECT ID FROM $wpdb->posts WHERE `post_title` = %s AND `post_type` = %s LIMIT 0,1", $username, $options['custom_post'] ) ); // same name
			...
			if ( ! $postID ) {
				...
				update_post_meta( $postID, 'creatorIP', self::get_ip_address() );
				update_post_meta( $postID, 'creatorFc', 'videowhisper_channel_user' );
			}
...
HTMLCODE;
		...
		static function videowhisper_channel_manage() {
			...
			if ( $postID ) {
				...
				global $wpdb;
				$existID = $wpdb->get_var( $wpdb->prepare( "SELECT ID FROM $wpdb->posts WHERE `ID` <> %d AND `post_title` = %s AND `post_type` = %s LIMIT 0,1", $postID, $name, $options['custom_post'] ) ); // same name, diff than postID
				...
				if ( $postID <= 0 && $channels_count >= $maxChannels ) {
					...
					if ( $postID ) {
						update_post_meta( $postID, 'creatorIP', self::get_ip_address() );
						update_post_meta( $postID, 'creatorFc', 'videowhisper_channel_manage' );
			...
			if ( !isset($_GET['editChannel']) && !isset($_GET['editPlaylist']) && !isset($_GET['reStream']) && !isset($_GET['offlineVideo']) ) {
				...
				if ( count( $channels ) ) {
					...
					foreach ( $channels as $channel ) {
						...
						$sql      = $wpdb->prepare( "SELECT * FROM $table_channels WHERE name = %s", $room );
						$channelR = $wpdb->get_row( $sql );

						if ( ! $channelR ) {
							$sql = $wpdb->prepare(
								"INSERT INTO `$table_channels` ( `owner`, `name`, `sdate`, `edate`, `rdate`,`status`, `type`) VALUES (%d, %s, %d, %d, %d, %d, %d)",
								$current_user->ID, $room, $ztime, $ztime, $ztime, 0, $rtype
							);
						} elseif ( $options['timeReset'] && $channelR->rdate < $ztime - $options['timeReset'] * 24 * 3600 ) { // time to reset in days
							$sql = $wpdb->prepare(
								"UPDATE `$table_channels` set type=%d, rdate=%d, wtime=0, btime=0 where name=%s",
								$rtype, $ztime, $room
							);
						} else {
							$sql = $wpdb->prepare(
								"UPDATE `$table_channels` set type=%d where name=%s",
								$rtype, $room
							);
						}
...
';
			...
			if ( $offlineVideo ) {
				...
				if ( shortcode_exists( 'videowhisper_postvideo_assign' ) ) {
							...
							$htmlCode .= '<h3 class="ui header">' . __( 'Offline Video', 'live-streaming' ) . ' #' . $offlineVideo . '</H3>';
							$htmlCode .= do_shortcode( "[videowhisper_postvideo_assign post_id=\"$offlineVideo\" meta=\"offline_video\"]" );

							$htmlCode .= '<p>' . __( 'Offline video plays in html5 player when channel is offline.', 'live-streaming' ) . '</p</div>';

				}

				// c

				// if ($offline_video) $addCode .= '<div class="item"><h3 class="ui ' . $options['interfaceClass'] . ' header">' . __('Current Offline Video', 'live-streaming') . '</h3> <div class="ui ' . $options['interfaceClass'] .' segment" style="min-width:320px">' . do_shortcode('[videowhisper_player video="' .$offline_video. '"]' . '</div></div>');
			}
...
HTMLCODE;
		...
		static function transcodeStream( $stream, $required = 0, $detect = 2, $convert = 1 ) {
			...
			global $wpdb;
			$postID = $wpdb->get_var( $wpdb->prepare( "SELECT MAX(ID) FROM $wpdb->posts WHERE post_title = %s and post_type = %s LIMIT 0,1", sanitize_file_name( $stream ), $options['custom_post'] ) );
...
static function path2url( $file, $Protocol = 'https://' ) {
		...
		static function updateOnline( $username, $room, $postID = 0, $type = 2, $current_user = '', $options = '' ) {
			...
			$sqlS    = $wpdb->prepare( "SELECT * FROM `$table_sessions` WHERE session=%s AND status < 2 AND rid=%d LIMIT 1", $s, $postID );
			...
			if ( ! $session ) {
				...
				$clientIP = self::get_ip_address();
				$sql = $wpdb->prepare(
					"INSERT INTO `$table_sessions` ( `session`, `username`, `uid`, `room`, `rid`, `roptions`, `rsdate`, `redate`, `message`, `sdate`, `edate`, `status`, `type`, `ip`) VALUES (%s, %s, %d, %s, %d, %s, %d, %d, %s, %d, %d, %d, %d, %s)",
					$s, $u, $uid, $r, $postID, $roptions, $rsdate, $redate, $m, $ztime, $ztime, 0, $type, $clientIP
				);
				$wpdb->query($sql);
				$session = $wpdb->get_row($sqlS);
				} else {
					...
					$sql = $wpdb->prepare(
						"UPDATE `$table_sessions` set edate=%d, rsdate=%d, redate=%d, roptions=%s WHERE id=%d LIMIT 1",
						$ztime, $rsdate, $redate, $roptions, $id
					);
					$wpdb->query($sql);
				}
				...
				$sqlC = $wpdb->prepare("SELECT * FROM $table_channels WHERE name=%s LIMIT 0,1", $session->room);
			...
			// update
			$sql = $wpdb->prepare("UPDATE `$table_channels` SET wtime = %d WHERE id = %d", $channel->wtime, $channel->id);
			$wpdb->query( $sql );
		...
		static function wp_ajax_vwls_htmlchat() {
			...
			$postID = $wpdb->get_var( $wpdb->prepare( "SELECT MAX(ID) FROM $wpdb->posts WHERE post_title = %s and post_type = %s LIMIT 0,1", $room, $options['custom_post'] ) );
			if ( ! $postID ) {
				throw new Exception( 'HTML Chat: Channel not found: ' . esc_html( $room ) );
			}
			...
			switch ( $_GET['task'] ) {
				...
				case 'sendTip':
					...
					if ( $error ) {
						...
						$sql = $wpdb->prepare(
							"INSERT INTO `$table_chatlog` ( `username`, `room`, `message`, `mdate`, `type`, `meta`, `user_id`) VALUES (%s, %s, %s, %d, %s, %s, %d)",
							$username, $room, $message, $ztime, '2', $metaS, $user_id
						);
						$wpdb->query( $sql );
				...
				case 'submitChat':
					...
					$sql = $wpdb->prepare(
						"INSERT INTO `$table_chatlog` ( `username`, `room`, `message`, `mdate`, `type`, `user_id`) VALUES (%s, %s, %s, %d, %s, %d)",
						$username, $room, $message, $ztime, '2', $user_id
					);
					$wpdb->query( $sql );
				...
				case 'getUsers':
					...
					$sql = $wpdb->prepare("SELECT * FROM `$table_sessions` WHERE room=%s AND status='0' AND type < 5 ORDER BY sdate ASC", $room);
				...
				case 'getChats':
					...
					if ( ! $disconnect ) {
						...
						$sql = $wpdb->prepare("SELECT * FROM `$table_chatlog` WHERE room=%s AND id > %d AND mdate > %d ORDER BY mdate DESC LIMIT 0,20", $room, $lastID, $sdate);
  ...
  <div class="ui left corner label">
    <i class="asterisk icon"></i>
				...
				$items = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM `$table_channels` WHERE status=1 ORDER BY edate DESC LIMIT %d, %d", $offset, $perPage ) );
				if ( $items ) {
					foreach ( $items as $item ) {
						$age = self::format_age( time() - $item->edate );
...
HTMLCODE;
		...
		static function widgetContent() {
			...
			if ( $items ) {
				foreach ( $items as $item ) {
					$count = $wpdb->get_results( $wpdb->prepare( "SELECT count(id) as no FROM `$table_viewers` WHERE status='1' AND room=%s", $item->room ) );

					$postID = $wpdb->get_var( $wpdb->prepare( "SELECT MAX(ID) FROM $wpdb->posts WHERE post_title = %s AND post_type='channel' LIMIT 0,1", $item->room ) );
					if ( $postID ) {
...
Turnkey Site Platform</a>.</p></div>';
		...
		static function columns_content_channel( $column_name, $post_id ) {
			...
			if ( $column_name == 'featured_image' ) {
				...
				global $wpdb;
				$postName = $wpdb->get_var( $wpdb->prepare( "SELECT post_title FROM $wpdb->posts WHERE ID = %d and post_type='channel' LIMIT 0,1", $post_id ) );
...
';
		...
		static function userPaidAccess( $userID, $postID ) {
			...
			$isBuyer         = $wpdb->get_col( $wpdb->prepare( "SELECT user_id FROM {$table_sessionsC} WHERE user_id=%d AND ref = 'buy_content' AND ref_id = %d AND creds < 0", $userID, $postID ) );
			if ( ! $isBuyer ) {
		...
		static function nav_menus() {
			...
			if ( ! $terms || is_wp_error( $terms ) ) {
				echo '<p>' . __( 'No items.', 'live-streaming' ) . '</p>';
				return;
			}
			...
			$page_links = paginate_links(
				array(
					...
					'format'    => '',
					'prev_text' => __( '&laquo;', 'live-streaming' ),
					'next_text' => __( '&raquo;', 'live-streaming' ),
					'total'     => $num_pages,
					'current'   => $pagenum,
				)
			);
	...
	<div id="taxonomy-<?php echo esc_attr( $taxonomy_name ); ?>" class="taxonomydiv">
		<ul id="taxonomy-<?php echo esc_attr( $taxonomy_name ); ?>-tabs" class="taxonomy-tabs add-menu-item-tabs">
			<li <?php echo ( 'most-used' == $current_tab ? ' class="tabs"' : '' ); ?>>
				...
				#tabs-panel-<?php echo esc_attr( $taxonomy_name ); ?>-pop">
					<?php _e( 'Most Used', 'live-streaming' ); ?>
				</a>
			</li>
			<li <?php echo ( 'all' == $current_tab ? ' class="tabs"' : '' ); ?>>
				...
				#tabs-panel-<?php echo esc_attr( $taxonomy_name ); ?>-all">
					<?php _e( 'View All', 'live-streaming' ); ?>
				</a>
			</li>
		...
		<div class="tabs-panel
			...
			<p class="quick-search-wrap">
				<input type="search" class="quick-search input-with-default-title" title="<?php _e( 'Search', 'live-streaming' ); ?>" value="<?php echo esc_attr( $searched ); ?>" name="quick-search-taxonomy-<?php echo esc_attr( $taxonomy_name ); ?>" />
				<span class="spinner"></span>
				<?php submit_button( __( 'Search', 'live-streaming' ), 'button-small quick-search-submit button-secondary hide-if-js', 'submit', false, array( 'id' => 'submit-quick-search-taxonomy-' . $taxonomy_name ) ); ?>
			</p>
			...
			<?php elseif ( ! empty( $searched ) ) : ?>
				<li><?php _e( 'No results found.', 'live-streaming' ); ?></li>
			<?php endif; ?>
			</ul>
		</div><!-- /.tabs-panel -->
		...
		<p class="button-controls">
			<span class="list-controls">
				...
				?>
			#taxonomy-<?php echo esc_attr( $taxonomy_name ); ?>" class="select-all"><?php _e( 'Select All', 'live-streaming' ); ?></a>
			</span>

			<span class="add-to-menu">
				<input type="submit"<?php wp_nav_menu_disabled_check( $nav_menu_selected_id ); ?> class="button-secondary submit-add-to-menu right" value="<?php esc_attr_e( 'Add to Menu', 'live-streaming' ); ?>" name="add-taxonomy-menu-item" id="<?php echo esc_attr( 'submit-taxonomy-' . esc_attr( $taxonomy_name ) ); ?>" />
				<span class="spinner"></span>
			</span>
		</p>
		...
		static function cron_schedules( $schedules ) {
			$schedules['min10'] = array(
				'interval' => 600,
				'display'  => __( 'Once every 10 minutes', 'live-streaming' ),
			);
			return $schedules;
		}
				...
				foreach ( $items as $item ) {
					...
					if ( isset($_GET['regenerateThumbs']) ) {
						...
						if ( file_exists( $filename ) ) {
							...
							$sql = $wpdb->prepare("UPDATE `$table_channels` SET status=%d WHERE name=%s", 1, $stream);
							$wpdb->query( $sql );

						} else {
							echo "<div class='warning'>Snapshot missing!</div>";
							$sql = $wpdb->prepare("UPDATE `$table_channels` SET status=%d WHERE name=%s", 0, $stream);
							$wpdb->query( $sql );

						}
					}

					$postID = $wpdb->get_var( $wpdb->prepare( "SELECT MAX(ID) FROM $wpdb->posts WHERE post_title = %s and post_type='channel' LIMIT 0,1", $item->name ) );

					if ( ! $options['anyChannels'] && ! $options['userChannels'] ) {
						if ( ! $postID ) {
							$wpdb->query( $wpdb->prepare( "DELETE FROM `$table_channels` WHERE name = %s", $item->name ) );
					...
					$broadcasting = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM `$table_sessions` WHERE room = %s ORDER BY edate DESC LIMIT 0, 100", $item->name ) );
					if ( $broadcasting ) {
...
<h3>Banning Channel</h3>
				...
				// delete post
				$postID = $wpdb->get_var( $wpdb->prepare( "SELECT MAX(ID) FROM $wpdb->posts WHERE post_title = %s and post_type='channel' LIMIT 0,1", $ban ) );
				if ( ! $postID ) {
				...
				$sql            = $wpdb->prepare("DELETE FROM `$table_sessions` WHERE name = %s", $ban);
				$wpdb->query( $sql );
...
<h3>Recent Channels</h3>
		...
		static function sessionUpdate( $username = '', $room = '', $broadcaster = 0, $type = 1, $strict = 1, $updated = 1, $userID = 0, $postID = 0, $options = null ) {
			...
			$sqlS    = $wpdb->prepare("SELECT * FROM $table_sessions WHERE session=%s AND status < 2 $cnd ORDER BY edate DESC LIMIT 0,1", $username);
			$session = $wpdb->get_row( $sqlS );

			if ( ! $session ) {

				$meta = serialize(['created_by' => 'sessionUpdate', 'postID' => $postID, 'userID' => $userID ]);
				$sql = $wpdb->prepare("INSERT INTO `$table_sessions` ( `session`, `username`, `room`, `message`, `sdate`, `edate`, `status`, `type`, `uid`, `rid`, `meta`) VALUES (%s, %s, %s, '', %d, %d, 1, %d, %d, %d, %s)", $username, $username, $room, $ztime, $ztime, $type, $userID, $postID, $meta);
			} else {

				//more updates, if needed
				$more = '';
				if ( $userID && $session->uid != $userID ) $more .= $wpdb->prepare(", uid=%d", $userID);
				if ( $postID && $session->rid != $postID ) $more .= $wpdb->prepare(", rid=%d", $postID);
				if ( $session->status != 1 ) $more .= ", status=1"; //0: pending

				$sql = $wpdb->prepare("UPDATE `$table_sessions` set edate=%d $more where id =%d", $ztime, $session->id);
			}
			$wpdb->query( $sql );
		...
		static function cleanSessions( $broadcaster = 0 ) {
			...
			$sql     = $wpdb->prepare("UPDATE `$table_sessions` SET status = 2 WHERE status < 2 AND edate < %d", $timed);
			$wpdb->query( $sql );
			...
			$sql     = $wpdb->prepare("DELETE FROM `$table_sessions` WHERE edate < %d", $exptime);
			$wpdb->query( $sql );
		}
		...
		static function streamSnapshot( $stream, $ipcam = false, $postID = 0 ) {
			...
			// table
			$sql = $wpdb->prepare("UPDATE `$table_channels` SET status=%d, edate=%d WHERE name=%s", $picType, $ztime, $stream);
			$wpdb->query( $sql );
		...
		*/
		...
		static function updateViewers( $postID, $room, $options ) {
			...
			global $wpdb;
			$table_viewers = $wpdb->prefix . 'vw_lwsessions';
			$viewers       = $wpdb->get_var( $wpdb->prepare( "SELECT count(id) AS no FROM `$table_viewers` WHERE status='1' AND rid=%d", $postID ) );
		...
		static function currentUserSession( $room ) {
			...
			$sql     = $wpdb->prepare("SELECT * FROM `$table_sessions` WHERE session=%s AND room=%s AND status='1' LIMIT 1", $username1, $room);
		...
		static function vwls_calls() {
			...
			// task: rtmp_status, rtmp_login, rtmp_logout 
			...
			switch ( $_GET['task'] ) {
							...
							if ( $rtpsession['streamPublish'] == 'true' && $userID && ! $disconnect && ! $transcoding ) {
								...
								if ( ! $disconnect ) {
									...
									*/
									...
									$sqlC    = $wpdb->prepare("SELECT * FROM $table_channels WHERE name=%s LIMIT 0,1", $r);
									$channel = $wpdb->get_row( $sqlC );
									...
									// update room
									$sql = $wpdb->prepare("UPDATE `$table_channels` set edate=%d, btime=%d where id=%d", $ztime, $btime, $channel->id);
									$wpdb->query( $sql );
							...
							if ( $rtpsession['streamPlay'] == 'true' && ! $disconnect ) {
								...
								*/
								...
								if ( $session->type >= '2' ) {
									...
									// update
									$sql = $wpdb->prepare("UPDATE `$table_channels` set wtime = %d where id = %d", $wtime, $channel->id);
									$wpdb->query( $sql );

									// update post
									$postID = $wpdb->get_var( $wpdb->prepare( "SELECT MAX(ID) FROM $wpdb->posts WHERE post_title = %s and post_type='channel' LIMIT 0,1", $r ) );
									if ( $postID ) {
					...
					if ( is_array( $users ) ) {
						foreach ( $users as $user ) {
							...
							if ( $user['role'] == '1' ) {
								...
								$postRow = $wpdb->get_row( $wpdb->prepare( "SELECT ID, post_author FROM $wpdb->posts WHERE post_title = %s and post_type=%s LIMIT 0,1", $r, $options['custom_post'] ) );
								if ( $postRow )
								{
								...
								*/
								...
								if ( $session->type >= 2 ) {
									...
									// update room
									$sql = $wpdb->prepare("UPDATE `$table_channels` set edate=%d, btime=%d where id=%d", $ztime, $channel->btime, $channel->id);
									$wpdb->query( $sql );

									// update post
									$postID = $wpdb->get_var( $wpdb->prepare("SELECT MAX(ID) FROM $wpdb->posts WHERE post_title=%s and post_type=%s LIMIT 0,1", $session->room, $options['custom_post']) );
				...
				case 'rtmp_logout':
					...
					if ($options['rtmpServer'] != 'wowza') break; // only required for specific rtmp server callbacks

					// rtmp server notifies client disconnect here
					$session = sanitize_file_name( $_GET['s'] ?? '' ); //prevent directory traversal with sanitize_file_name
					if ( ! $session ) {
						exit;
					}
					...
					if ( file_exists( $filename1 ) ) {
						echo esc_html( unlink( $filename1 ) ); // remove session file
					}
					?>
				...
				case 'rtmp_login':
					...
					global $wpdb;
					$wpdb->flush();
					$postID = $wpdb->get_var( $wpdb->prepare( "SELECT MAX(ID) FROM $wpdb->posts WHERE post_title = %s and post_type = %s LIMIT 0,1", sanitize_file_name( $channel ), $options['custom_post'] ) );
					...
					if ( ! $invalid ) {
						if ( $broadcaster == '1' ) {
							...
							if ( $key == $validKey ) {
								...
								if ( ! $channelR ) {
									$sql = $wpdb->prepare( "INSERT INTO `$table_channels` ( `owner`, `name`, `sdate`, `edate`, `rdate`, `status`, `type` ) VALUES ( %d, %s, %d, %d, %d, %d, %d )", $broadcasterID, $room, $ztime, $ztime, $ztime, 0, 1 );
								} elseif ( $options['timeReset'] && $channelR->rdate < $ztime - $options['timeReset'] * 24 * 3600 ) { // time to reset in days
									$sql = $wpdb->prepare( "UPDATE `$table_channels` SET edate=%d, type=1, rdate=%d, wtime=0, btime=0 WHERE name=%s", $ztime, $ztime, $room );
								} else {
									$sql = $wpdb->prepare( "UPDATE `$table_channels` SET edate=%d WHERE name=%s", $ztime, $room );
								}
					...
					$filename1 = $dir . "/_sessions/$session";
					if ( file_exists( $filename1 ) ) {
						echo esc_html( implode( '', file( $filename1 ) ) );
						if ( $broadcaster ) {
							echo '&role=' . wp_kses_post( $broadcaster );
						}
					} else {
						echo esc_html( 'VideoWhisper=1&login=0&missingSession=' . sanitize_file_name( $session ) . '&invalid=' . $invalid . '&verified=' . $verified . '&wrongKey=' . $wrongKey . '&k=' . sanitize_text_field( $key ) . '&p=' . count( $p ) );
					}
					...
					if ( $verified ) {
						...
						if ( $options['webStatus'] == 'auto' ) {
							...
							$updateOptions        = 1;
							echo esc_html( '&webStatus=' . $options['webStatus'] );
						}
